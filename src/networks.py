...
###### IRINA GRIGORESCU
######
###### This is the CoTAN network
###### Refer to https://github.com/m-qiang/CoTAN for original code
########################################################################
import torch
...
###################################################
################################################### Code adapted from https://github.com/m-qiang/CoTAN
###################################################
class VFNet(nn.Module):
    """
    A 3D U-Net to predict multiscale stationary velocity fields (SVFs).
...
class CoTAN(nn.Module):
    ...
    Note: I have changed the original code to use tanh and gaussian smoothing on the velocity fields
    ...
    def __init__(self, layers=[16, 32, 32, 32, 32], C=16, M=4, R=3, sigma=0.5, device='cpu'):
    ...
    def forward(self, x, T, age, vol):
        ...
        # clip the large deformation if needed
        vel_field = torch.tanh(vel_field)
        # smooth velocity field
        vel_field = self.gaussian(vel_field)  # gaussian smooth
        ...
        for n in range(N):
            ...
            x = x + h * v  # deformation
            # print(n, v.shape) # n torch.Size([1, |V|, 3]) # |V|=135708
...
class CoTANPial(nn.Module):
    ...
    def interpolate(self, x, vel_field):
        # rescale the vertices between -1 and 1
        grid = x / self.rescale
        grid = grid.repeat(self.RM, 1, 1).unsqueeze(dim=-2).unsqueeze(dim=-2).flip(-1)
        v = F.grid_sample(vel_field, grid, mode='bilinear',
                          padding_mode='border', align_corners=True)
